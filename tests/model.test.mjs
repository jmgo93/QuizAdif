...
import { examPoints, examGrade } from '../js/quiz.js';
...
test('normaliza y conserva la jerarquía',()=>{const q=normalizeQuestion({...raw,questionType:'Caso aplicado'});assert.equal(q.scope,'specific');assert.equal(q.topic,'Vía');assert.equal(q.correctIndex,2);assert.equal(q.questionType,'Caso aplicado');});
...
test('el examen resta un punto por cada tres fallos',()=>{assert.equal(examPoints(7,3),6);assert.equal(examGrade(7,3,10),6);assert.equal(examPoints(7,0),7);});
